...
const stringify = (object, depth, spacesCount = 4) => {
    ...
    for (const [key, value] of Object.entries(object)) {
        result.push(`${' '.repeat(spacesCount * depth)}${key}: ${stringify(value, depth + 1)}`)
    }
...
const formatter = (objs) => {
    ...
    const stylish = (objs, depth) => {
        ...
        for (const obj of objs) {
            const { key, type } = obj
            if (type === 'added') {
                const value = stringify(obj.value, depth + 1)
                result.push(`${' '.repeat(spacesCount * depth - 2)}` + `+ ${key}: ${value}`) 
            } else if (type === 'removed') {
                const value = stringify(obj.value, depth + 1)
                result.push(`${' '.repeat(spacesCount * depth - 2)}` + `- ${key}: ${value}`)
            } else if (type === 'changed') {
                const oldValue = stringify(obj.oldValue, depth + 1)
                const newValue = stringify(obj.newValue, depth + 1)
